...
const areAllActivitiesCompleted = (activities) => {
  return Object.values(activities).every(activity => activity.status === "Completato");
}
...
const moveOrderToArchive = async (orderId) => {
    try {
        ...
        if (!areAllActivitiesCompleted(order.activity)) {
            throw new Error('Ordine non completato e quindi non può essere archiviato');
        }   
...
router.post('/:id', async (req, res) => {
    ...
    try {
        ...
        console.error(`Errore durante l'archiviazione dell'ordine ${orderId}:`, error.message);
        if (error.message === 'Ordine non completato e quindi non può essere archiviato') {
            res.status(400).send(error.message);
        } else if (error.message === 'Ordine non trovato') {
            res.status(404).send(error.message);
        } else {
            res.status(500).send('Errore durante l\'archiviazione dell\'ordine: ' + error.message);
        }
    }
});
